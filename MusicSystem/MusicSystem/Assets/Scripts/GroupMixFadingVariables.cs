...
public class GroupMixFadingVariables : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        /*if (Input.GetKeyDown("o"))
        {
            ...
            {
                additionalLayerVolume = 0;
                StartCoroutine(FadeMixerGroup.StartFade(MainMixer, exposedMixerParameter, 1.5f, additionalLayerVolume));
            }
            else
            {
    ...
    {
        //first we take the current distance and convert it to a value between 0 and 1
        float currentDistance = Mathf.InverseLerp(minDistance, maxDistance, inputdistance);
        /*then we take the value between 0 and 1 and convert it to a value between 0 and -80
         and assign that value to additionalVolumeLayer*/
        additionalLayerVolume = Mathf.Lerp(0.0f, -80.0f, currentDistance);
        //Debug.Log("Distance converted to fader level is " + additionalLayerVolume);
        //finally we set the level of the additional layer mixer group to the value of additionalLayerVolume
        MainMixer.SetFloat(exposedMixerParameter, additionalLayerVolume);
    }
}
